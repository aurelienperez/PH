...
set.seed(1)
covariates <- tibble(y=y,sexe=ifelse(donnees_ajust$Sexe=="F",1,0),intercept=rep(1,dim(donnees_ajust)[1]))
x_reg <- reg(x2, y ~ sexe + intercept ,data=covariates, weight=weights, stepsEM = 3,every=1,scale=100)
```

```{r}
plot(y[1:111],weights[1:111])
curve(dens(x_reg,x)*exp(x_reg@reg$pars[1]),col='red',add=T)
```
